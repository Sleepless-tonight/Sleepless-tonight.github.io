...
8. 面向对象
1、面向对象的一个主要特性就是隐藏实现（不去暴露非必要暴露的对象成员），即一个对象的成员的可访问范围是必须要进行控制的，而公开的通常是方法，通常的做法是将对象属性私有，通过 get、set 方法来获得对象的属性。易于扩展、可复用、易维护、构建复杂系统 （继承 支持了 复用，多态 支持了 扩展，封装 支持了 构建复用
2、OOP设计一条基本的准则是“为状态的变化使用数据成员，为行为的变化使用多性形”。
3、记住多形性只能通过方法调用才能表现出来
   
9. 运行期类型鉴定
为理解RTTI在Java里如何工作，首先必须了解类型信息在运行期是如何表示的。

10. Logger
大家不要再在方法里写：System.out.println("查询到的储位列表数据：" + list);
这个很耗费系统性能，且不易维护。
可以用如下方式代替：

    1）、可以在类里声明一个 Logger 对象：
    		Logger logger = LoggerFactory.getLogger(WmsStorageController.class);

    2）、在需要控制台进行输出处：
    		logger.info("查询到的储位列表数据1: {}",list);
    		logger.error("查询到的储位列表数据2: {}" , list);
    		logger.debug("查询到的储位列表数据3: {}",list);
    info、error、debug 表示不同的日志输出等级
